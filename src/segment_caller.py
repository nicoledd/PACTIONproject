...
def solveSegmentPaction(args):  

    k = len(args.c)
    snvDf = pd.read_csv(args.s, sep='\t')
    s = snvToIdxDict(snvDf) # snv name -> idx
    nsamples = snvDf.columns.str.startswith('sample').sum()
    trees = []
    for currSeg in range(k):
        currSnvDf = snvDf.loc[snvDf['segment'] == currSeg]
        currCnaDf = pd.read_csv(args.c[currSeg], sep='\t')
        T = segmentTree(currSnvDf, currCnaDf, s, nsamples)
        '''
        plt.figure(figsize=(15,15))
        pos = graphviz_layout(T, prog="dot")
        nx.draw(T, with_labels = True, font_size=10, pos=pos)
        plt.savefig('segment_' + str(currSeg) + '.png')
        '''
        trees.append(T)

    finalT = solveProgressivePaction(trees, nsamples)
